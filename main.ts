...
export default class CloneVaultPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.addCommand({
      id: "clone-vault-with-name",
      name: "Clone Vault (Prompt for name)",
      callback: async () => {
        const modal = new NamePromptModal(this.app, async (vaultName: string) => {
          if (!vaultName) {
          ...
          try {
            ...
            await copyObsidianFolder(currentVaultPath, newVaultPath);
            ...
            new Notice(`Vault "${vaultName}" cloning successful!`);
          } catch (err) {
            console.error(err);
            new Notice("Vault cloning failed!");
          }
        });
        modal.open();
      },
...
async function copyObsidianFolder(currentVaultPath: string, newVaultPath: string) {
  const oldConfigFolder = path.join(currentVaultPath, ".obsidian");
  const newConfigFolder = path.join(newVaultPath, ".obsidian");
...
async function copyFolderStructure(
  ...
  for (const entry of entries) {
    ...
    if (foldersToCopy.includes(entry.name)) {
      ...
      await copyFolderStructure(folderPath, newFolderPath, foldersToCopyStr);
    }
  }
}
